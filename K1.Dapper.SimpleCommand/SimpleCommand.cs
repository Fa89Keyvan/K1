...
namespace K1.Dapper.SimpleCommand
{
    ...
    public static partial class SimpleCommand
    {
        ...
        public static IEnumerable<T> GetList<T>(this IDbConnection connection, object whereConditions, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            var sb = new StringBuilder();
            var whereprops = TypeHelper.GetAllProperties(whereConditions).ToArray();
            sb.Append("Select ");
            //create a new empty instance of the type to get the base properties
            BuildSelect(sb, GetScaffoldableProperties<T>().ToArray());
        ...
        public static TKey Insert<TKey, TEntity>(this IDbConnection connection, TEntity entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            if (keytype == typeof(Guid))
            {
                ...
                {
                    var newguid = GuidHelper.SequentialGuid();
        ...
        public static int Update<TEntity>(this IDbConnection connection, TEntity entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            CacheService.StringBuilderCache(masterSb, $"{typeof(TEntity).FullName}_Update", sb =>
            {
        ...
        {
            var masterSb = new StringBuilder();
            CacheService.StringBuilderCache(masterSb, $"{typeof(T).FullName}_Delete", sb =>
            {
        ...
        {
            var masterSb = new StringBuilder();
            CacheService.StringBuilderCache(masterSb, $"{typeof(T).FullName}_DeleteWhere{whereConditions?.GetType()?.FullName}", sb =>
            {
                var currenttype = typeof(T);
                var name = GetTableName(currenttype);
        ...
        {
            var masterSb = new StringBuilder();
            CacheService.StringBuilderCache(masterSb, $"{typeof(T).FullName}_DeleteWhere{conditions}", sb =>
            {
