...
namespace K1.Dapper.SimpleCommand.Test
{
    ...
    public class Tests
    {
        ...
        private IDbConnection GetOpenConnection()
        {
            ...
            {
                connection = new SqlConnection(Program._connectionString);
        ...
        {
            SetDialect(Dialect.SQLServer);
            GetDialect().IsEqualTo(Dialect.SQLServer);
            SetDialect(Dialect.PostgreSQL);
            GetDialect().IsEqualTo(Dialect.PostgreSQL);
        }
        ...
        public void TestMultiInsertASync()
        {
            using (var connection = GetOpenConnection())
            {
                connection.InsertAsync(new User { Name = "TestMultiInsertASync1", Age = 10 }).Wait();
                connection.InsertAsync(new User { Name = "TestMultiInsertASync2", Age = 10 }).Wait();
                connection.InsertAsync(new User { Name = "TestMultiInsertASync3", Age = 10 }).Wait();
                connection.InsertAsync(new User { Name = "TestMultiInsertASync4", Age = 11 }).Wait();
